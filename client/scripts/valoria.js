(function(){function r(e,n,t){function o(i,f){if(!n[i]){if(!e[i]){var c="function"==typeof require&&require;if(!f&&c)return c(i,!0);if(u)return u(i,!0);var a=new Error("Cannot find module '"+i+"'");throw a.code="MODULE_NOT_FOUND",a}var p=n[i]={exports:{}};e[i][0].call(p.exports,function(r){var n=e[i][1][r];return o(n||r)},p,p.exports,r,e,n,t)}return n[i].exports}for(var u="function"==typeof require&&require,i=0;i<t.length;i++)o(t[i]);return o}return r})()({1:[function(require,module,exports){
  ...
  class Valoria {
    ...
    onMediaStream(stream, callId) {
      const socket = this.socket;
      console.log("onMediaStream: ", callId);
      // 加入聊天室
    ...
    onIceCandidate(event) {
      ...
      const socket = this.valoria.socket;
      const thisVal = this.valoria;
    ...
    createOffer(thisVal){
      ...
      const peerConnection = thisVal.peerConnection;
      const callId = thisVal.callId;
      peerConnection.createOffer(
        function (offer) {
          //如果offer创建成功，则将其设置为本地描述
          //并通过套接字连接发送它以在另一方启动peerConnection
          peerConnection.setLocalDescription(offer);
          socket.emit("offer", JSON.stringify(offer), callId);
        },
        function (err) {
    ...
    createAnswer(thisVal, offer) {
      ...
      peerConnection.createAnswer(
        function (answer) {
          ...
          socket.emit("answer", JSON.stringify(answer), callId);
        },
        function (err) {
    ...
    onAnswer(answer, thisVal) {
      ...
      thisVal.localICECandidates.forEach((candidate) => {
        console.log(`>>> Sending local ICE candidate (${candidate.address})`);
        //通过websocket发送ice候选人
        socket.emit("candidate", JSON.stringify(candidate), callId);
      });
      //重置本地ICE候选者的缓冲区。
    ...
    onTrack(event) {
      console.log(event);
      console.log("onTrack <<< Received new stream from remote. Adding it...");
      //更新远程视频源
      event.currentTarget.valoria.onCallAnswered(event.streams[0])
      event.currentTarget.valoria.connected = true;
      //从视频中删除加载的gif
      //关闭初始共享网址栏
      //更新连接状态
      //隐藏字幕状态文本
    ...
    call(userId, myStream, cb){
      ...
      console.log("HOW MANY AM I CALLING?")
      this.socket.emit('Call User', {toUserId: userId, userId: this.user.id, toUsername: this.peers[userId].username});
      // this.onMediaStream(stream);
      this.active[userId] = {userId: userId, username: this.peers[userId].username};
    }
  
    answer(d, myStream, cb){
      this.onCallAnswered = function(stream){
        if(cb && typeof cb === 'function'){
          cb(stream);
        }
      ...
      this.callId = d.callId;
      this.onMediaStream(myStream, d.callId);
